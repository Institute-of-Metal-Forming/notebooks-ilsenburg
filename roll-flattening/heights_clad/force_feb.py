...
def testy_uppercase_phi_neutral_point(coeff_a, reduction):
    ...
    return (1 / coeff_a) * (
        np.tan(
            0.5 * np.arctan(np.sqrt(reduction / (1 - reduction)))
...
def testy_function_three(coeff_a, reduction):
    ...
    def testy_function_one():
        ...
        def core_equation(x, switch=1):
            return (1 + (coeff_a**2) * (x**2)) * np.exp(
                switch * 2 * coeff_a * np.arctan(coeff_a * x)
            )

        first_part_result = integrate.quad(
            lambda x: core_equation(x),
            0,
            testy_uppercase_phi_neutral_point(coeff_a, reduction),
        )
        second_part_result = integrate.quad(
            lambda x: core_equation(x, -1),
            testy_uppercase_phi_neutral_point(coeff_a, reduction),
            testy_uppercase_phi_entry(coeff_a, reduction),
        )
        return (
            first_part_result[0]
            + (1 - reduction)
            * np.exp(2 * coeff_a * np.arctan(np.sqrt(reduction / (1 - reduction))))
...
def testy_function_four(coeff_a, reduction):
    ...
    def testy_function_two():
        ...
        def core_equation(x, switch=1):
            return (
                (1 + (coeff_a**2) * (x**2))
                * np.exp(switch * 2 * coeff_a * np.arctan(coeff_a * x))
                * x
            )

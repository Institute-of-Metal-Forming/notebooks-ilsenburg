...
def contact_length(height_at_start: float, height_end: float, roll_radius_current: float):
    """Contact length with the roll with same lower and upper roll"""
    return np.sqrt(roll_radius_current * (height_at_start - height_end) - (((height_at_start - height_end) ** 2) / 4))
...
def flow_stress_diff(x: float, coefficients: FreibergFlowStressCoefficients, strain_rate: float, temperature: float, starting_height: float, flow_stress_second):
    """
...
def roll_force_feb(kf_m_clad, width_wp, roll_radius_current, height_at_start, height_end, factor_1):
    delta_h = height_at_start - height_end
    return 1.15 * kf_m_clad * width_wp * np.sqrt(roll_radius_current * delta_h) * factor_1

def roll_torque_feb(roll_radius_current, height_at_start, height_end, kf_m_clad, width_wp, factor_2):
    return 2 * roll_radius_current * (height_at_start ** 2) * kf_m_clad * width_wp * factor_2 / height_end

def roll_force_and_torque(radius,
                          upper_material_coeff, lower_material_coeff,
                          height_layer_upper: float, height_layer_lower: float, temperature: float, width: float,
                          roll_velocity: float, friction_coefficient: float, roll_gap: float):
    ...
    strain_rate_equivalent = equivalent_strain_rate(roll_velocity, contact_arc, strain_overall)
    flow_stress_upper = flow_stress(0, upper_material_coeff, strain_rate_equivalent, temperature)
    flow_stress_lower = flow_stress(0, lower_material_coeff, strain_rate_equivalent, temperature)
    # Sort
    ...
    height_bonding = fsolve(flow_stress_diff, x0=np.array([0.0019]), args=(coeff_soft, strain_rate_equivalent, temperature, start_height_soft, flow_stress_harder))[0]
    ...
    end_strain_soft = strain_phi(height_at_start=start_height_soft, height_end=end_height_soft)
    end_strain_hard = strain_phi(height_at_start=start_height_hard, height_end=end_height_hard)
    end_flow_stress_soft = flow_stress(end_strain_soft, coeff_soft, strain_rate_equivalent, temperature)
    end_flow_stress_hard = flow_stress(end_strain_hard, coeff_hard, strain_rate_equivalent, temperature)
    weight = 2  # geometrical vs. analytical weighting can be changed here (1 vs 2)
    mean_flow_stress = (flow_stress_softer + weight * end_flow_stress_soft + flow_stress_harder + weight * end_flow_stress_hard) / (4 + (weight - 1) * 2)
    ...
    roll_force = roll_force_feb(mean_flow_stress, width, radius, height_start, roll_gap, f1)
    roll_torque = roll_torque_feb(radius, height_start, roll_gap, mean_flow_stress, width, f2)
